 ...
 */
function takeString(requestData, key, maxLen, required = true) {
...
exports.createReport = onCall( async (request) => {
  ...
  const reportedUserId = takeString(data, 'reportedUserId', 128);
  const reason = takeString(data, 'reason', 120);
  const details = takeString(data, 'details', 2000, false);
  const reportedContent = takeString(data, 'reportedContent', 4000, false);
  const reportedContentId = takeString(data, 'reportedContentId', 256, false);
  const reportedContentType = takeString(data, 'reportedContentType', 64, false);
  const reportedContentParentId = takeString(data, 'reportedContentParentId', 256, false);
...
exports.sendAdminNotification = onCall(async (request) => {
  ...
  const title = takeString(data, 'title', 100);
  const body = takeString(data, 'body', 500);
  const segment = takeString(data, 'segment', 40, false) || 'all';
  const targetUid = takeString(data, 'targetUid', 200, false); // segment == 'user'
  // optional in-app navigation route
  const targetRoute = takeString(data, 'targetRoute', 120, false);
  if (!title || !body) {
    throw new HttpsError('invalid-argument',
 ...
 * @return {GoogleGenerativeAI} The Gemini client.
 */
...
/**
 * Sanitize the reply from the AI.
 ...
 * @param {string} msg The message to classify.
 * @return {string} The intent of the message.
 */
 ...
 * @param {string} nativeLang The native language.
 * @param {string} level The learning level.
...
/**
 * Resolve the daily limit for a given key.
 * @param {string} key The key to resolve the limit for.
 * @return {number} The daily limit.
 */
function resolveDailyLimit(key) {
  const defaults = {vocabotSend: 200, vocabotAnalyzeGrammar: 50, aiTranslate: 150};
  try {
    if (JSON.parse(process.env.AI_CONFIG || '{}')) {
      const cfg = JSON.parse(process.env.AI_CONFIG || '{}');
      if (cfg[key]) {
...
/**
 * Check the daily quota for a given key.
 * @param {object} request The request object.
 * @param {string} key The key to check the quota for.
 */
...
exports.vocabotSend = onCall(async (request) => {
  if (!request.auth) throw new HttpsError('unauthenticated', 'Giriş gerekli');
...
exports.vocabotAnalyzeGrammar = onCall(async (request) => {
  if (!request.auth) throw new HttpsError('unauthenticated', 'Giriş gerekli');
  await requirePremium(request.auth.uid);
...
exports.aiTranslate = onCall(async (request) => {
  if (!request.auth) throw new HttpsError('unauthenticated', 'Giriş gerekli');
  await requirePremium(request.auth.uid);
...
if (GENAI_API_KEY) {
  try {
    new GoogleGenerativeAI(GENAI_API_KEY);
  } catch (e) {
 ...
 * @param {string} nativeLanguage The native language.
 * @return {object} The fallback quiz.
 */
...
exports.vocabotGrammarQuiz = onCall(async (request) => {
  if (!request.auth) {
   ...
   * @param {string} v The string to clamp.
   * @param {number} max The maximum length.
   * @param {string} def The default value if the string is null.
