...
class _PracticeSpeakingDetailScreenState extends State<PracticeSpeakingDetailScreen> {
  ...
  String _recognized = '';
  int _currentIndex = 0;
  List<SpeakingEvaluation> _results = [];
  DateTime? _recordStart;
  String? _enLocaleId; // İngilizce locale seçimi
  // Yeni: canlı metrikler & ses seviyesi & otomatik ileri
  ...
  Future<void> _initEngines() async {
    ...
    // Cihazdaki İngilizce locale'leri bul
    try {
      final locales = await _speech.locales();
      // Önce en_US arıyoruz, yoksa herhangi bir en_* alıyoruz
  ...
  Future<void> _toggleRecord() async {
    if(!_sttAvailable){
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Konuşma tanıma kullanılamıyor.')));
      return;
    }
    if(_recording){
      await _speech.stop();
  ...
  void _updateLiveMetrics({bool finalResult=false}){
    ...
    if(finalResult && _autoNext && _liveSimilarity>=85){
      // kısa gecikme ile otomatik ileri
  ...
  // Eksik kalan yardımcılar geri eklendi
  ...
  List<String> _buildSuggestions(double sim, List<String> fillers, String target, String spoken){
    final tips = <String>[];
    if(sim < 60) tips.add('Hedef cümledeki ana kelimeleri daha net yakalamaya çalış.');
    if(fillers.isNotEmpty) tips.add('Dolgu kelimeleri azalt: ${fillers.join(', ')}');
    if(spoken.split(' ').length < target.split(' ').length*0.6) tips.add('Cümleyi tamlamayı dene, bazı bölümler eksik.');
    if(sim >= 85) tips.add('Harika akıcılık! Ufak telaffuz farkları için ritim/vurguya odaklan.');
    if(tips.isEmpty) tips.add('İyi iş! Bir kez daha deneyerek daha akıcı ol.');
    return tips;
  }

  double _similarityRatio(String a, String b){
    // Gelişmiş: normalizasyon + token Levenshtein + s��raya dayalı bigram bonusu
    final na = _normalizeForCompare(a);
    final nb = _normalizeForCompare(b);
    if(na.isEmpty && nb.isEmpty) return 1;
    if(na == nb) return 1; // tamamen aynı
    ...
    // Aşırı küçük farklar için micro‑penalty yumuşatma (örn. tek harf / çoğul s)
    if(base < 1){
      // Son token çoğul farkı (cat vs cats)
    ...
    if(score >= 0.995) score = 1; // tolerans: çok küçük farkları 100 kabul et
    return score;
  }
  ...
  // Eksik: konuşma sonrası küçük temizlik fonksiyonu
      ...
      return Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Hedef Sayısı: ${prompt.targets.length}'),
            Text('Ortalama Benzerlik: ${avgSim.toStringAsFixed(1)}%'),
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (c,i){
                  ...
                  return ListTile(
                    dense: true,
                    title: Text('Cümle ${i+1} �� ${r.similarity.toStringAsFixed(1)}%'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _playTarget, icon: const Icon(Icons.volume_up_rounded), tooltip: 'Dinle'),
          Switch(
            value: _autoNext,
            onChanged: (v)=> setState(()=> _autoNext=v),
            thumbIcon: WidgetStateProperty.resolveWith((s)=> const Icon(Icons.fast_forward, size:16)),
          )
        ],
      ),
      body: Stack(
                      ...
                      children:[
                        Row(children:[
                          Container(
                            ...
                            child: Text('Hedef ${_currentIndex+1}/${prompt.targets.length}', style: const TextStyle(color: Colors.blueAccent,fontWeight: FontWeight.w600)),
                          ),
                          const Spacer(),
                            ...
                            children: List.generate(20, (i){
                              ...
                              return Expanded(
                                child: AnimatedContainer(
                                  ...
                                  decoration: BoxDecoration(
                                    color: on? Colors.tealAccent : Colors.grey.withValues(alpha:.25),
                        ...
                        AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            color: _recording? Colors.red.withValues(alpha: .08): Colors.grey.withValues(alpha: .06),
                            border: Border.all(color: _recording? Colors.redAccent: Colors.grey.shade600, width: 1),
                            borderRadius: BorderRadius.circular(12),
                          ),
                          constraints: const BoxConstraints(minHeight:80),
                          child: _recognized.isEmpty
                            ? Text(_recording? 'Konuş ve sistem gerçek zamanlı ölçsün...' : 'Kayda başla ve cümleyi söyle.', style: TextStyle(color: Colors.grey.shade400))
                            : Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children:[
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Wrap(spacing:12, runSpacing:4, children: [
                              ...
                              _MetricChip(label:'Filler', value: _liveFillers.toString()),
                              _MetricChip(label:'Kelime', value: (_recognized.trim().isEmpty?0:_recognized.trim().split(RegExp(r'\\s+')).length).toString()),
                            ]),
                          ],
                        ),
                        const SizedBox(height:8),
                        // ...existing evaluation suggestion block remains (kept below)
                      ],
                    ),
                  ),
                ),
                  ...
                  children:[
                    ...
                    Expanded(
                      child: FilledButton.icon(
                        ...
                        label: Text(_recording? 'Durdur' : 'Kaydı Başlat'),
                      ),
                    ),
                    const SizedBox(width:12),
                    FilledButton(
                      onPressed: _currentIndex == prompt.targets.length-1? _showSummary : _next,
                      child: Text(_currentIndex == prompt.targets.length-1? 'Özet' : 'İleri'),
                    )
                  ],
                ),
              )
...
// Yeni yardımcı metric chip widget
...
// Arka plan: hafif partikül + degrade (kaldı, performansı düşük)
